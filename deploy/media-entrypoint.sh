...
readiness_key=${SMOKE_GPU_READINESS_VERIFY_KEY:-/run/secrets/gpu_readiness_verify_key}
readiness_key_id=${SMOKE_GPU_READINESS_KEY_ID:-gpu-readiness}
executor_key=${SMOKE_GPU_EXECUTOR_VERIFY_KEY:-/run/secrets/gpu_executor_verify_key}
executor_key_id=${SMOKE_GPU_EXECUTOR_KEY_ID:-gpu-executor}
...
test -r "$readiness_key" || { echo "GPU readiness verification key is missing: $readiness_key" >&2; exit 78; }
test -r "$executor_key" || { echo "GPU executor verification key is missing: $executor_key" >&2; exit 78; }
...
PYTHONPATH=/opt/smoke-detect/receipt python3 /opt/smoke-detect/receipt/validate_gpu_receipt.py \
  ...
  --readiness-verify-key "$readiness_key" --signing-key-id "$readiness_key_id" \
  --executor-verify-key "$executor_key" --executor-signing-key-id "$executor_key_id" || exit 78
