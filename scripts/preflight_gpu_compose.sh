...
key_file=${SMOKE_GPU_READINESS_VERIFY_KEY_FILE:-$REPO_ROOT/deploy/secrets/gpu_readiness_verify_key.txt}
...
if [[ ! -r "$key_file" ]]; then
  echo "GPU Compose preflight failed: readiness verification key is unreadable: $key_file" >&2
  echo "Provision the public key from the site release store." >&2
  exit 78
fi
...
export SMOKE_GPU_READINESS_VERIFY_KEY_FILE="$key_file"
